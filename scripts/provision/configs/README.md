...
This directory contains configuration files for all Cathedral services. These configurations are generated by the provisioning system and customized for specific environments.
...
./scripts/cathedral.sh provision configure --env production
...
./scripts/cathedral.sh deploy config
...
chmod 600 *.toml
chown cathedral:cathedral *.toml

# SSH keys - very restrictive
chmod 400 /etc/cathedral/keys/*
chown cathedral:cathedral /etc/cathedral/keys/*
...
sudo chown -R cathedral:cathedral /etc/cathedral/
sudo chmod -R 600 /etc/cathedral/*.toml
sudo chmod -R 400 /etc/cathedral/keys/
```
...
This configuration system provides a secure, scalable foundation for deploying Cathedral services across different environments while maintaining consistency and security best practices.