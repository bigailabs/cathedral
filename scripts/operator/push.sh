...
SOURCE_IMAGE="basilica/basilica-operator"
TARGET_IMAGE="ghcr.io/one-covenant/basilica-operator"
IMAGE_TAG="latest"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --source-image SOURCE     Source Docker image (default: basilica/basilica-operator)"
            echo "  --target-image TARGET     Target Docker image (default: ghcr.io/one-covenant/basilica-operator)"
...
if ! docker images --format "table {{.Repository}}:{{.Tag}}" | grep -q "^${SOURCE_IMAGE_FULL}$"; then
    ...
    echo "Please build the image first using: ./scripts/operator/build.sh" >&2
    exit 1
fi
