...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo ""
            echo "Rollout basilica-storage deployment in K3s cluster"
            echo ""
            echo "Options:"
            echo "  --namespace NAMESPACE     Kubernetes namespace (default: basilica-storage)"
            echo "  --deployment DEPLOYMENT   Deployment name (default: fuse-daemon)"
            ...
            echo "  $0"
            echo "  $0 --follow-logs"
            echo "  $0 --namespace basilica-storage --deployment basilica-storage"
            exit 0
            ;;
        *)
...
kubectl rollout restart daemonset/"$DEPLOYMENT" -n "$NAMESPACE"

echo ""
echo "Waiting for rollout to complete (timeout: $WAIT_TIMEOUT)..."
kubectl rollout status daemonset/"$DEPLOYMENT" -n "$NAMESPACE" --timeout="$WAIT_TIMEOUT"
...
if [[ "$FOLLOW_LOGS" == "true" ]]; then
    ...
    kubectl logs -n "$NAMESPACE" daemonset/"$DEPLOYMENT" --tail=50 -f
fi
