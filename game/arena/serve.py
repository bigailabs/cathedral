...
def _primary_ip() -> str | None:
    """Best-effort primary non-loopback IPv4 (the WSL IP a Windows browser must use
    when localhost forwarding is off). No packets are sent; the connect just selects
...
def startup_urls(port: int, ip: str | None = "") -> list[str]:
    """The URLs to advertise: always localhost, plus the LAN/WSL IP when detectable
    (so a Windows browser has a working address even when localhost won't forward)."""
    if ip == "":
        ip = _primary_ip()
    urls = [f"http://localhost:{port}/home"]
    if ip:
        urls.append(f"http://{ip}:{port}/home")
    return urls
...
def serve(port: int = 8800) -> None:
    ...
    if len(urls) > 1:
        print(f"  From a Windows browser (if localhost won't load): {urls[1]}")
...
<div class="game">
  ...
  <main class="stage">
    <section class="arena" id="arena">
      ...
      <div id="actionFlash" class="actionFlash">TARGET LOCK</div>
      <div class="arenaContract"><span>Active contract</span><b id="arenaTarget">routing target</b><small id="arenaRoute">risk and bounty loading</small><div class="bar"><i id="arenaRiskFill"></i></div></div>
      <div id="nodes"></div>
    ...
    <aside class="sidebar">
      <div class="mission">
        ...
        <div class="missionHud"><span>Current mission</span><b id="missionObjective">Route scan</b><small id="missionHint">Wait for replay tasks to load.</small></div>
...
function flashText(kind,msg){const f=$('actionFlash');f.className='actionFlash '+(kind||'ok');f.textContent=msg;void f.offsetWidth;f.classList.add('show')}
...
function renderNodes(){const root=$('nodes');root.innerHTML='';state.tasks.forEach((t,i)=>{const p=pos(i,state.tasks.length);const risk=targetRisk(t);const n=document.createElement('div');n.className='node risk-'+riskBand(risk)+(i===state.selected?' active':'')+(state.cleared.has(i)?' cleared':'')+(state.rejected.has(i)?' rejected':'');n.style.left=`calc(${p.left}% - 66px)`;n.style.top=`calc(${p.top}% - 41px)`;n.title=t.objective;n.onclick=()=>select(i);n.innerHTML=`<div class="sn">SN${t.target.netuid} / ${t.expected_family}</div><strong>${t.target.name}</strong><small>+${Math.round(t.bounty_weight*1000)} bounty / risk ${risk}</small><span class="tag">${state.cleared.has(i)?'sealed':state.rejected.has(i)?'blocked':riskBand(risk)}</span>`;root.appendChild(n)})}
...
function phaseInstruction(){return {probe:['Probe the target','Read the task and price the risk before spending energy.'],encode:['Encode the invariant','Align the required proof family so replay can verify it.'],solve:['Solve the witness','Generate an artifact with the exact fields this target requires.'],replay:['Replay the proof','Run the deterministic verifier before any ledger write.'],attest:['Attest the run','Bind the accepted replay to a local receipt.'],seal:['Seal the reward','Submit the attested artifact and let the ledger score it.'],sealed:['Target sealed','Pick another live target or run the season back.']}[phases[state.phase]]||['Route scan','Wait for replay tasks to load.']}
function renderMission(){const ins=phaseInstruction();let hint=ins[1];const next=nextAction();const c=actionCost(next);if(state.energy<=c[0])hint='Energy is low: press 9 Cooldown before continuing.';else if(state.sweep>=75)hint='Validator sweep is hot: press 9 Cooldown to avoid losing a gate.';else if(state.cleared.has(state.selected))hint='This target is already sealed: click the core to advance.';else hint+=' Next: '+actionLabel()+'.';$('missionObjective').textContent=ins[0];$('missionHint').textContent=hint}
...
async function runAgentSolve(mode='valid'){const t=current();const solved=await fetch('/api/scanner/agent/solve?task_id='+encodeURIComponent(t.task_id)+'&miner_hotkey='+encodeURIComponent(player)+'&mode='+encodeURIComponent(mode)).then(r=>r.json());if(!solved.submission){throw new Error(solved.error||'agent solve failed')}state.pendingProofs[t.task_id]=solved.submission;delete state.replayOk[t.task_id];delete state.attested[t.task_id];const cls=mode==='valid'?'ok':'bad';const label=mode==='valid'?'agent produced proof artifact ':mode==='report_only'?'agent wrote report-only decoy ':'agent forged corrupt artifact ';flashText(cls,mode==='valid'?'WITNESS READY':mode==='report_only'?'REPORT ONLY':'FORGED CLAIM');log(label+solved.artifact_sha256.slice(0,12),cls)}
async function runReplay(){const t=current();const proof=state.pendingProofs[t.task_id];if(!proof){state.blocked++;state.combo=1;state.phase=2;state.rejected.add(state.selected);state.lastGates={task_matches:null,nonce_matches:null,family_aligned:null,decode_map_present:false,replay_succeeds:false,attestation_receipt:null};flash('reject');flashText('bad','REPLAY FAIL');log('REPLAY FAILED: no pending proof artifact; run Solve first','bad');return false}const payload={...proof,miner_hotkey:player,report:'dry-run replay through Subnet Breaker UI'};const verdict=await fetch('/api/scanner/replay',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)}).then(r=>r.json());state.lastGates={...(verdict.gates||{}),attestation_receipt:false};if(verdict.accepted){state.replayOk[t.task_id]=true;state.rejected.delete(state.selected);flashText('ok','REPLAY PASS');log('REPLAY ACCEPTED: '+verdict.replay_target_id+' / no ledger write','ok');return true}state.blocked++;state.combo=1;state.phase=2;state.rejected.add(state.selected);delete state.pendingProofs[t.task_id];delete state.replayOk[t.task_id];delete state.attested[t.task_id];flash('reject');flashText('bad','REPLAY FAIL');log('REPLAY REJECTED: '+(verdict.reasons||[verdict.error||'unknown']).join(', '),'bad');return false}
async function runAttestation(){const t=current();const proof=state.pendingProofs[t.task_id];if(!proof){state.blocked++;state.combo=1;flash('reject');flashText('bad','ATTEST FAIL');log('ATTEST FAILED: no proof artifact; run Solve first','bad');return false}if(!state.replayOk[t.task_id]){state.blocked++;state.combo=1;flash('reject');flashText('bad','ATTEST FAIL');log('ATTEST FAILED: replay gate has not passed','bad');return false}const verdict=await fetch('/api/scanner/attest',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...proof,miner_hotkey:player,report:'local attestation dry-run through Subnet Breaker UI'})}).then(r=>r.json());state.lastGates={...(verdict.gates||{}),attestation_receipt:!!verdict.attested};if(verdict.attested&&verdict.receipt){state.attested[t.task_id]=verdict.receipt;proof.claim={...(proof.claim||{}),attestation:verdict.receipt};flashText('attest','ATTEST RECEIPT');log('ATTESTED: simulated local TEE receipt '+verdict.receipt.artifact_sha256.slice(0,12)+' bound to claim','attestLog');return true}state.blocked++;state.combo=1;flash('reject');flashText('bad','ATTEST FAIL');log('ATTEST REJECTED: '+(verdict.reasons||[verdict.error||'unknown']).join(', '),'bad');return false}
async function seal(){const t=current();const proof=state.pendingProofs[t.task_id];if(!proof){state.blocked++;state.combo=1;flash('reject');flashText('bad','SEAL FAIL');log('REJECTED: no pending proof artifact; run Solve first','bad');return}if(!state.replayOk[t.task_id]){state.blocked++;state.combo=1;flash('reject');flashText('bad','SEAL FAIL');log('REJECTED: replay gate has not passed','bad');return}if(!state.attested[t.task_id]){state.blocked++;state.combo=1;flash('reject');flashText('bad','SEAL FAIL');log('REJECTED: attestation receipt has not been bound','bad');return}const payload={...proof,miner_hotkey:player,report:'sealed with local attestation receipt through Subnet Breaker UI'};const verdict=await fetch('/api/scanner/submit-attested',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)}).then(r=>r.json());delete state.pendingProofs[t.task_id];delete state.replayOk[t.task_id];delete state.attested[t.task_id];if(verdict.accepted){state.phase=6;state.heat=Math.max(0,state.heat-20);state.combo=Math.min(2.5,state.combo+.15);flash('breached');flashText('ok','PROOF SEALED');log('SEALED: backend accepted '+verdict.replay_target_id+' / focus '+state.combo.toFixed(1),'ok')}else{state.blocked++;state.combo=1;flash('reject');flashText('bad','SEAL FAIL');log('REJECTED: '+(verdict.reasons||[verdict.error||'unknown']).join(', '),'bad')}await syncState(false);await syncBenchmark();const board=await fetch('/api/scanner/leaderboard').then(r=>r.json());log('your ledger score '+(state.score/1000).toFixed(1)+' / kill '+Math.round(state.killRate*100)+'%','warn');if(board.miners&&board.miners[0]&&board.miners[0].miner_hotkey!==player)log('global #1 '+board.miners[0].miner_hotkey+' score '+board.miners[0].score,'warn');if(verdict.accepted){const n=nextOpen();if(n<0){$('end').classList.add('show')}else{state.selected=n;state.phase=0;state.lastGates=null;log('advanced to next subnet target','warn')}}}
async function act(name){if(!state.tasks.length||state.campaignEnded)return;if(name==='decoy'){state.heat=Math.max(0,state.heat-28);state.energy=Math.min(100,state.energy+12);advanceSweep('decoy');flashText('ok','COOLDOWN ROUTE');log('cooldown route deployed: heat and sweep dumped, tempo lost','ok');render();return}if(state.cleared.has(state.selected)){const n=nextOpen();if(n<0){finishCampaign('SEASON CLEARED','Accepted proofs were written to the scanner ledger.');return}flashText('ok','NEXT TARGET');select(n);return}const needed={probe:0,encode:1,solve:2,replay:3,attest:4,submit:5,report:2,forge:2}[name];if(state.phase!==needed){state.blocked++;state.combo=1;state.sweep=Math.min(100,state.sweep+18);flash('reject');flashText('bad','GATE REJECTED');log('gate rejected: run the chain in order','bad');render();return}const c=actionCost(name);if(state.energy<=c[0]){state.blocked++;state.sweep=Math.min(100,state.sweep+14);flash('reject');flashText('bad','AGENT EXHAUSTED');log('agent exhausted: cooldown before continuing','bad');render();return}fire();if(name==='probe'){spend(c[0],c[1]);state.phase=1;flashText('ok','TARGET LOCK');log('task fetched; target risk '+targetRisk(current())+' now prices every gate','ok')}if(name==='encode'){spend(c[0],c[1]);state.phase=2;flashText('ok','INVARIANT ARMED');log('family gate aligned: '+current().expected_family,'ok')}if(name==='solve'){spend(c[0],c[1]);await runAgentSolve();state.phase=3;log('witness fields prepared: '+current().required_fields.join(', '),'ok')}if(name==='report'){spend(c[0],c[1]);await runAgentSolve('report_only');state.phase=3;log('report-only claim prepared; replay should reject it','warn')}if(name==='forge'){spend(c[0],c[1]);await runAgentSolve('bad_witness');state.phase=3;log('forged witness prepared; replay should catch this','warn')}if(name==='replay'){spend(c[0],c[1]);if(await runReplay())state.phase=4}if(name==='attest'){spend(c[0],c[1]);if(await runAttestation())state.phase=5}if(name==='submit'){spend(c[0],c[1]);await seal()}advanceSweep(name);render()}
