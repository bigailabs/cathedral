...
def main() -> int:
    ...
    tmp.close()
    store = Store(tmp.path) if hasattr(tmp, "path") else Store(tmp.name)

    family = "gentest"
    tier = 1
    nat_cnf, _ = gen_planted_3sat(1, 5, 21, method="biased")
    inj_seed = 0x1234_5678_9abc_def0 & ((1 << 63) - 1)
    inj_cnf, _ = gen_planted_3sat(inj_seed, 5, 21, method="biased")
