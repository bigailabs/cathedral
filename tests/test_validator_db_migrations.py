...
@pytest.mark.asyncio
async def test_connect_migrates_legacy_agent_submissions_before_sat_indexes(tmp_path) -> None:
    ...
    try:
        ...
        raw.execute(
            """
            CREATE UNIQUE INDEX idx_agent_unique
        ...
        raw.execute(
            ...
            INSERT INTO agent_submissions (
                ...
                'sub-legacy', 'hk', 'synthetic_boolean_v1', '', 'bundle',
                0, '', 'sig', 'miner', '', 1,
    ...
    try:
        ...
        cur = await conn.execute("PRAGMA index_list(agent_submissions)")
        indexes = {row[1] for row in await cur.fetchall()}
        assert {
            "idx_agent_unique",
            "idx_agent_unique_sat_challenge",
            "idx_agent_sat_challenge_seq_no",
            "idx_agent_sat_challenge_seq_scan",
        } <= indexes

        cur = await conn.execute(
            "SELECT id, sat_challenge_id, seq_no, attestation_mode "
            "FROM agent_submissions WHERE id = 'sub-legacy'"
        )
        assert await cur.fetchone() == ("sub-legacy", None, None, "polaris")

        await conn.execute(
            "UPDATE agent_submissions SET status = 'pending_solution' "
            "WHERE id = 'sub-legacy'"
        )
        await conn.commit()
    finally:
