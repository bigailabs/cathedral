...
def build_app(
    *,
    ...
    @app.on_event("startup")
    async def _prewarm_visibility_caches():
        if not visibility_cold_async:
            return

        def _warm() -> None:
            for limit in (20, 50, 250):
                try:
                    recent_cache.get(limit, lambda limit=limit: _recent_payload(None, None, limit))
                except Exception as exc:
                    print(f"[visibility_cache] recent_prewarm_failed limit={limit} error={exc!r}")
            try:
                pm_summary_cache.get(50, lambda: _perminer_summary(50))
            except Exception as exc:
                print(f"[visibility_cache] pm_summary_prewarm_failed error={exc!r}")
            try:
                explain_cache.get("5test", lambda: _leaderboard_explain_payload("5test"))
            except Exception as exc:
                print(f"[visibility_cache] explain_prewarm_failed error={exc!r}")

        threading.Thread(target=_warm, name="visibility-cache-prewarm", daemon=True).start()
