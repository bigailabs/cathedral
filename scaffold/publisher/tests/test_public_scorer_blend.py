...
class FakeStore:
    ...
    def query(self, sql, params):
        ...
        if "FROM metagraph_hotkeys" in sql:
            cutoff = params[2]
            return [{"hotkey": hk, "updated_at_iso": self._meta_updated}
...
def test_newer_zero_revokes_older_positive(store):
    ...
    r1 = _make_report(
        ...
        source=source, epoch=1, complete=True, now=now,
    )
    ...
    r2 = _make_report(
        ...
        source=source, epoch=2, complete=True, now=now,
    )
...
def test_default_disabled(monkeypatch):
    ...
    for k in ("CATHEDRAL_EXTERNAL_SCORES_ENABLED", "CATHEDRAL_EXTERNAL_SCORES_FRACTION"):
        monkeypatch.delenv(k, raising=False)
...
def test_status_ignores_stale_snapshot(store):
    ...
    r = _make_report(
        [("5MINER_0", 0.8)],
        source=source, epoch=1, complete=True, now=old_time,
    )
