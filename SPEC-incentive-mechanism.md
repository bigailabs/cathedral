...
| `target_count` snapshotting | **Not snapshotted (intentional)** | `target_count` is read from the current backend config at weight-setting time. It acts as a live category capacity cap / upper-limit control. Changing it retroactively affects the payout rate of existing unvested CUs in that category. |
...
pub struct IncentiveConfig {
    ...
    /// It is read from the current backend config at weight-setting time and acts as a
...
pub struct GpuCategoryConfig {
    ...
    /// NOTE: NOT snapshotted per CU row. Read from current backend config at weight-setting time.
...
**Step 1 — Current Category Capacity (from backend config)**
```
// target_count is a live operator control read from CURRENT backend config.
