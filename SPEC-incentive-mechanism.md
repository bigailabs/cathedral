...
| Feature toggle | **`cu_generator_enabled: bool`** | Boolean on `ValidatorConfig` (default `false`). When `true`, the validator runs as the primary and spawns CU Generator plus slash detection. When `false` (default), the validator only computes weights from shared data (follower mode). New validators detect the new incentive system by fetching config from `GET /v1/incentive/config` on `basilica-api`: if config exists, use the new CU-based path. During rollout, an explicit `404` / empty response means "not configured yet" and may still use legacy delivery-based weights. Once the new system is live, transient read failures must fail the weight-setting attempt for that epoch rather than falling back. Older validator binaries continue using the old delivery endpoints unchanged. |
...
Validation Loop (~15min)  [EXISTING — crates/basilica-validator/src/miner_prover/]
  ...
  │     (if config exists → use CU/RU path; during rollout only, explicit 404/empty may use legacy path;
  │      transient read failures do NOT fall back once live)
...
- **FR-4**: `GET /v1/incentive/config` returns 404 or empty when the incentive system is not configured. This is an explicit rollout signal, not a generic error condition. New validators may use it to fall back to legacy delivery-based weights only before the new system is declared live.
...
2. **Rust (vesting engine)**: Iterate rows, compute per-CU vesting, aggregate per-hotkey:
   ...
   fn compute_vested_cus(
       ...
       let mut hotkey_vested: HashMap<String, Decimal> = HashMap::new();

       for row in rows {
           let window_ms = (row.window_hours as i128) * 3_600_000;  // per-row vesting window in ms
           let vest_end = row.earned_at + Duration::milliseconds(window_ms as i64);
           let overlap_start = row.earned_at.max(epoch_start);
           let overlap_end = vest_end.min(epoch_end);
           let overlap_ms = (overlap_end - overlap_start).num_milliseconds().max(0);
           let vesting_fraction = Decimal::from(overlap_ms) / Decimal::from(window_ms);
...
- **Response** (200):
  ...
  ```
- **Response** (404): Incentive system not configured. During rollout, new validators may use this explicit signal to fall back to legacy delivery-based weights. Once the new system is live, validators are expected to receive a configured response; transient failures must not be treated as "not configured".
...
if raw_usd_required_epoch > usd_emission_capacity:
    scale_factor = usd_emission_capacity / raw_usd_required_epoch  // always < 1.0
    For each hotkey:
...
burn_rate = clamp(burn_rate, 0.0, 0.99)  // never burn 100% — capped at 99%
...
match self.api_client.get_incentive_config().await {
    ...
    Err(BasilicaApiError::NotConfigured) => {
        // ROLLOUT-ONLY LEGACY PATH: backend explicitly signaled that the incentive
        // system is not configured yet. Older validator binaries continue using the
        // old delivery endpoints unchanged.
    ...
    Err(e) => {
        // DO NOT fall back to legacy on transient errors (timeout, 5xx, auth issues).
        // Once the new system is live, failing closed is safer than mixing two weight models.
        return Err(e.into());
    }
}
```
...
- **FR-8**: All validators (primary and follower) use the same weight-setting code path. Incentive config is fetched from `basilica-incentive` at each epoch. If the API returns config, use the new CU-based path. During rollout only, an explicit `404` / empty response may use the legacy delivery-based path. Transient failures (`timeout`, `5xx`, auth issues) must not fall back to legacy.
...
5. **Regression**: Verify legacy delivery-based path continues unchanged during rollout when backend returns explicit 404 for `GET /v1/incentive/config`
