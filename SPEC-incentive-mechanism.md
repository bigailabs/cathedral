...
|---|---|---|
| Database for CU Ledger | **PostgreSQL** (owned by `basilica-incentive`) | Centralized, HA, data-resilient. Validators access it via the dedicated `basilica-incentive` REST API (same `BasilicaApiClient` signing pattern as `/v1/weights/miner-delivery` and `/v1/prices/tokens`). Validators never connect to the DB directly. |
...
| "Total number of Asking" | **`basilica-incentive` config parameter** | Configured as target GPU counts per category in the incentive service config, served via `GET /v1/incentive/config`. Not fetched from any external API. |
...
| Feature toggle | **`cu_generator_enabled: bool`** | Boolean on `ValidatorConfig` (default `false`). When `true`, the validator runs as the primary — spawns CU Generator and slash detection. When `false` (default), the validator only computes weights from shared data (follower mode). New validators detect the new incentive system by fetching config from `GET /v1/incentive/config`: if the `basilica-incentive` service returns config, use the new CU-based weight path. During rollout, an explicit `404` / empty response means "not configured yet" and may still use legacy delivery-based weights. Once the new system is live, new validators must NOT fall back on transient config-read failures (`timeout`, `5xx`, auth issues); they should fail the weight-setting attempt for that epoch. Older validator binaries continue using the old delivery endpoints unchanged. |
| Validator roles | **Single primary validator** | Only one primary validator (ours) generates CUs and slashes. `basilica-incentive` enforces write-auth via hotkey allowlist on `POST` endpoints — non-authorized validators get 403. All validators (including primary) read CU/RU data + config from the incentive service and compute weights independently. Deterministic: same config + same data + same math = same weights. |
| Incentive config location | **`basilica-incentive`-owned** | All incentive parameters (gpu_categories, window_hours, max_cu_value_usd, revenue_share_pct, slash_pct) live in `basilica-incentive` and are served via `GET /v1/incentive/config`. Validators fetch config from the service when needed and use it transiently — no local storage. This means follower validators need zero configuration. |
...
Validation Loop (~15min)  [EXISTING — crates/basilica-validator/src/miner_prover/]
  ...
  ├── fetches incentive config from basilica-incentive (GET /v1/incentive/config)
  ...
  └── detects slash conditions (node completely lost during rental)
        │
        ▼ (via basilica-incentive REST API)
  Incentive Service (`basilica-incentive`)  [NEW — dedicated backend service]
  ├── owns incentive config
  ├── owns CU ledger
  ├── owns RU ledger
  ├── enforces primary-validator write auth
  ├── serves validator read APIs
  ...
  All incentive data ──▼── (via basilica-incentive REST API)
...
Incentive Pool Inputs (all from basilica-incentive: GET /v1/incentive/config):
...
Both run identical weight-setting code. The only difference: the primary validator also spawns the CU Generator and slash detection tasks. Follower validators need zero configuration — they detect the new system by successfully fetching config from `basilica-incentive`.

### Data Flow Summary
```
CU path:  Validation Loop → [local SQLite] Availability Log → CU Generator → [REST API] basilica-incentive CU Ledger → [REST API] Weight Setter → Bittensor Chain
RU path:  Billing System (telemetry charges) → credit_transactions → basilica-incentive RU Generator → ru_ledger → [REST API] Weight Setter → Bittensor Chain
Config:   basilica-incentive config → [REST API] GET /v1/incentive/config → Weight Setter + CU Generator
```
...
The full incentive configuration lives in `basilica-incentive` and is served to all validators via `GET /v1/incentive/config`. How it is configured internally (TOML, environment variables, database table, admin API) is a service implementation detail outside the scope of this validator spec.
...
## Module 3.5: `basilica-incentive` Service API

**Location**: `basilica-backend/crates/basilica-incentive/` (new service)

### Purpose
REST API endpoints that `basilica-incentive` must implement to serve incentive config and CU/RU ledger data to validators. Endpoints are split into **write** (primary validator only) and **read** (all validators).
...
## Module 3.5R: `basilica-incentive` RU Ledger API

**Location**: `basilica-backend/crates/basilica-incentive/` (same service, alongside CU Ledger API)
...
### Purpose
Periodic task (~1 hour, primary validator only) that reads availability log entries from local SQLite since the last run, fetches incentive config from `basilica-incentive`, then submits CU rows to the centralized CU ledger via `POST /v1/incentive/cus`. Uses **actual observation span** for CU computation — CU amount is derived from the time range of actual availability observations, NOT from elapsed time since the last generator run.
...
every ~1 hour (hardcoded 3600s interval, not configurable):
  ...
  0. incentive_config = api_client.get_incentive_config()
     (fetched from `basilica-incentive` each tick — not stored locally)
