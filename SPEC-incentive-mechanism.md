...
| Revenue Units (RU) | **`basilica-incentive`-generated from rental revenue data** | RU rows are generated by `basilica-incentive` from actual per-period rental revenue facts/events tied to rentals. The billing/backend rental-revenue pipeline remains the monetary source of truth; the incentive service consumes period-level revenue amounts or deltas and writes `ru_ledger` rows. It must not derive RU from user balance credit transactions and must not rely on cumulative rental totals without periodization. `ru_amount` = actual USD rental revenue generated for that period. Payout: `vested_fraction * ru_amount * revenue_share_pct / 100` — no per-category dilution. Subject to global emission cap. |
...
| Incentive config location | **`basilica-incentive`-owned** | All incentive parameters (gpu_categories, window_hours, max_cu_value_usd, revenue_share_pct, slash_pct) live in `basilica-incentive` and are served to validators via `GET /v1/incentive/config` on `basilica-api`. Validators fetch config when needed and use it transiently — no local storage. This means follower validators need zero extra configuration. |
...
The full incentive configuration lives in `basilica-incentive` and is served to validators via validator-facing `GET /v1/incentive/config` on `basilica-api`. How it is configured internally (TOML, environment variables, database table, admin API) is a backend implementation detail outside the scope of this validator spec.
...
pub struct GpuCategoryConfig {
    ...
    /// Used for per-row payout calculations.
    /// NOTE: Snapshotted per CU row at earn time — config changes only affect future CUs.
    pub price_usd: f64,
}
```
...
struct NewCuRow {
    ...
    window_hours: f64,             // from basilica-incentive config at earn time
    price_usd: f64,                // from basilica-incentive gpu_categories[cat].price_usd at earn time
...
RU rows are generated by **`basilica-incentive`** (not the validator). The backend rental-revenue pipeline remains the source of truth for the underlying revenue; the incentive service consumes per-period rental revenue facts/events and writes RU rows. The RU generation process:

1. Periodically scans recent per-period rental revenue facts/events from the backend revenue source of truth
2. For each revenue period, resolves the miner identity (hotkey, miner_uid, node_id) via the rental record
3. Creates an `ru_ledger` row with `ru_amount` = the actual USD rental revenue generated for that period
4. Snapshots `revenue_share_pct` and `window_hours` from the current incentive config

The RU generator must also persist a durable progress cursor so it can continue correctly after restarts. That cursor should be based on the billing revenue source's stable ordering field: typically the last processed event timestamp, or a unique monotonic event ID if one exists and is the better source cursor. RU insertion must remain idempotent so reprocessing an already-seen event does not create duplicate `ru_ledger` rows.
...
| `ru_amount` | Actual USD rental revenue generated for this period |
...
- **FR-2**: `ru_amount` reflects actual USD rental revenue generated for the period, not user balance credit transactions, rental list-price estimates, or raw cumulative totals
...
- **FR-6**: RU generator progress is durable. It stores the last processed source-event timestamp or unique event ID so restarts continue from the correct point without missing events
...
RU rows are generated internally by `basilica-incentive`, not submitted by validators. The initial implementation uses a periodic checkpointed ingestion job that reads backend-owned per-period rental revenue facts/events and creates corresponding `ru_ledger` rows. The job must persist a durable cursor using the source event timestamp or unique event ID, whichever is the correct stable ordering field exposed by the billing service, so it can continue correctly after restarts. It must not use `credit_transactions`, and it must not rely on cumulative rental values without deriving correct per-period revenue deltas. A future event-driven ingestion path can be considered later if needed, but it is not part of the initial rollout plan.
...
### Purpose
Periodic task (~1 hour, primary validator only) that reads availability log entries from local SQLite since the last run, fetches incentive config from validator-facing `GET /v1/incentive/config` on `basilica-api`, then submits CU rows to the centralized CU ledger via `POST /v1/incentive/cus`. Uses **actual observation span** for CU computation — CU amount is derived from the time range of actual availability observations, NOT from elapsed time since the last generator run.
...
9. Create `incentive/cu_generator.rs` — fetches config from API, reads local SQLite availability log, uses observation-span-based CU calculation, submits CU rows via `basilica-incentive`
10. Spawn CU Generator in `service.rs` (gated on `config.cu_generator_enabled == true`)
11. `basilica-incentive`: Implement RU generation from backend-owned per-period rental revenue facts/events, persist the last processed source cursor, and generate `ru_ledger` rows idempotently
