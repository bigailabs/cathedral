...
| Revenue Units (RU) | **`basilica-incentive`-generated from billing data** | RU rows are generated by `basilica-incentive` from actual credit deductions in the billing system (`credit_transactions`). The billing system remains the monetary source of truth; the incentive service consumes successful debit facts and writes `ru_ledger` rows. `ru_amount` = actual USD charged. Payout: `vested_fraction * ru_amount * revenue_share_pct / 100` — no per-category dilution. Subject to global emission cap. |
...
Validation Loop (~15min)  [EXISTING — crates/basilica-validator/src/miner_prover/]
  ...
  └── exposes successful debit facts to basilica-incentive for RU generation
...
RU path:  Billing System (telemetry charges) → credit_transactions → basilica-incentive RU Generator → ru_ledger → [public HTTP] basilica-api `/v1/incentive/rus` → Weight Setter → Bittensor Chain
...
RU rows are generated by **`basilica-incentive`** (not the validator). The billing system remains the source of truth for the underlying debits in `credit_transactions`; the incentive service consumes that billing-owned data and writes RU rows. The RU generation process:
...
| **Generated by** | Validator (CU Generator) | `basilica-incentive` (from billing data) |
...
### Purpose
Validator-facing API for the RU ledger. Unlike CU endpoints (where validators POST rows), RU rows are generated internally by the backend from billing data. Validators only need to **read** RU data for weight computation.
...
crates/basilica-validator/
  ...
  migrations/
    ...
    #   - RU generation logic (consumes billing debit facts, writes ru_ledger)
...
11. `basilica-incentive`: Implement RU generation from billing-owned debit facts — consumes successful rental charge data and generates `ru_ledger` rows
