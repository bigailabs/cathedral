...
def load_producer_from_env(
    env: Mapping[str, str] | None = None,
) -> tuple[WeightPolicyProducerConfig, Ed25519PrivateKey] | None:
    """Return producer config + signer when the signing key env var is set."""
    values = os.environ if env is None else env
    signing_hex = values.get(_WEIGHT_POLICY_SIGNING_KEY_ENV, "").strip()
    if not signing_hex:
        return None
    try:
    ...
    burn_uid_raw = values.get("CATHEDRAL_WEIGHT_POLICY_BURN_UID", "204").strip()
    burn_uid = int(burn_uid_raw) if burn_uid_raw else None
    cfg = WeightPolicyProducerConfig(
        network=values.get("CATHEDRAL_WEIGHT_POLICY_NETWORK", "finney"),
        netuid=int(values.get("CATHEDRAL_WEIGHT_POLICY_NETUID", "39")),
        ...
        limit=int(values.get("CATHEDRAL_WEIGHT_POLICY_LIMIT", "1000")),
        task_family_weights=_resolve_task_family_weights(None, env=values),
...
def _resolve_task_family_weights(
    ...
    *,
    ...
    raw_json = values.get("CATHEDRAL_WEIGHT_POLICY_TASK_FAMILY_WEIGHTS_JSON")
    if raw_json:
        try:
    ...
    raw_boolean = values.get("CATHEDRAL_WEIGHT_POLICY_SYNTHETIC_BOOLEAN_V1_WEIGHT")
    if raw_boolean is None:
        raw_boolean = values.get("CATHEDRAL_SYNTHETIC_BOOLEAN_V1_WEIGHT")
