...
class HealthSnapshot(BaseModel):
    ...
    # Weight path indicator. The validator now runs a single local path:
    # "A" = local aggregation (pull_loop + weight_loop, hardcoded burn).
    # The signed-publisher-vector relay (Path B) was removed. The field is
    # retained as a stable /health key (always "A" once startup sets it;
    # None = uninitialised).
