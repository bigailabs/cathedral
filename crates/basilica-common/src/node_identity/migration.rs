...
#[cfg(feature = "sqlite")]
use crate::node_identity::SqliteIdentityStore;
...
mod tests {
    ...
    async fn create_test_db() -> Result<sqlx::SqlitePool> {
        ...
        sqlx::query("INSERT INTO node_health (node_id, status) VALUES (?, ?)")
            .bind("legacy-node-1")
            .bind("active")
            .execute(&pool)
            .await?;
        ...
        sqlx::query("INSERT INTO ssh_sessions (node_id) VALUES (?)")
            .bind("legacy-node-1")
            .execute(&pool)
            .await?;
    ...
    async fn test_scan_legacy_ids() {
        ...
        assert!(legacy_ids.contains(&"legacy-node-1".to_string()));
    ...
    async fn test_migrate_batch() {
        ...
        let legacy_ids = vec!["legacy-node-1".to_string(), "old-node-abc123".to_string()];
        ...
        assert!(mappings.contains_key("legacy-node-1"));
    ...
    async fn test_update_references() {
        ...
        // since node_health has node_id as PRIMARY KEY
        ...
        for (legacy_id, new_uuid) in &mappings {
            let count: (i64,) =
                sqlx::query_as("SELECT COUNT(*) FROM ssh_sessions WHERE node_id = ?")
                    .bind(new_uuid.to_string())
                    .fetch_one(&pool)
                    .await
                    .expect("Should query");
    ...
    async fn test_full_migration_workflow() {
        ...
        // Only scan ssh_sessions table since node_health has PRIMARY KEY constraint
        let config = MigrationConfig {
            scan_targets: vec![ScanTarget {
                table: "ssh_sessions".to_string(),
                id_column: "node_id".to_string(),
        ...
        // First, manually scan node_health to find legacy IDs
        let legacy_ids: Vec<String> = sqlx::query_scalar(
            "SELECT node_id FROM node_health WHERE node_id NOT LIKE '%-%-%-%-%'",
        )
        .fetch_all(&pool)
        .await
        ...
        // since node_health has node_id as PRIMARY KEY and can't be updated
        let update_config = MigrationConfig {
            scan_targets: vec![ScanTarget {
                table: "ssh_sessions".to_string(),
                id_column: "node_id".to_string(),
    ...
    async fn test_migration_report_summary() {
        ...
        reference_updates.insert("node_health".to_string(), 15);
        ...
        assert!(summary.contains("node_health: 15 rows"));
