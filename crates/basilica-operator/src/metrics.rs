...
fn ensure_init() {
    INIT.call_once(|| {
        ...
        describe_gauge!("basilica_operator_active_jobs_total", "Active running jobs per namespace");

        // Rentals
        describe_counter!("basilica_operator_rentals_reconciles_total", "Total rental reconciles");
        describe_counter!("basilica_operator_rentals_created_total", "Rental pods created by controller");
        describe_counter!("basilica_operator_rentals_status_transitions_total", "Rental status transitions");
...
pub fn record_job_reconcile(ns: &str, name: &str, created: bool, from: &str, to: &str, start: Instant) {
    ensure_init();
    counter!("basilica_operator_jobs_reconciles_total", "namespace" => ns.to_string()).increment(1);
    if created {
        counter!("basilica_operator_jobs_created_total", "namespace" => ns.to_string()).increment(1);
    }
    if from != to {
        counter!(
    ...
    histogram!("basilica_operator_job_reconcile_duration_seconds", "namespace" => ns.to_string()).record(secs);
}
...
pub fn record_rental_reconcile(ns: &str, name: &str, created: bool, from: &str, to: &str, start: Instant) {
    ensure_init();
    counter!("basilica_operator_rentals_reconciles_total", "namespace" => ns.to_string()).increment(1);
    if created {
        counter!("basilica_operator_rentals_created_total", "namespace" => ns.to_string()).increment(1);
    }
    if from != to {
        counter!(
...
pub fn record_rental_extension(ns: &str, approved: bool) {
    ...
    if approved {
        ...
        counter!("basilica_operator_rentals_extension_denied_total", "namespace" => ns.to_string()).increment(1);
    }
}
...
pub fn record_job_active_change(ns: &str, prev_active: bool, new_active: bool) {
    ...
    match (prev_active, new_active) {
        (false, true) => gauge!("basilica_operator_active_jobs_total", "namespace" => ns.to_string()).increment(1.0),
        (true, false) => gauge!("basilica_operator_active_jobs_total", "namespace" => ns.to_string()).decrement(1.0),
        _ => {}
    }
}
...
pub fn record_rental_active_change(ns: &str, prev_active: bool, new_active: bool) {
    ensure_init();
    match (prev_active, new_active) {
        (false, true) => gauge!("basilica_operator_active_rentals_total", "namespace" => ns.to_string()).increment(1.0),
