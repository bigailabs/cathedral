...
impl MinerPrometheusMetrics {
    ...
    pub fn new() -> Result<Self> {
        ...
        describe_gauge!(
            "cathedral_miner_nodes_total",
            "Total number of managed nodes"
        );
        describe_gauge!("cathedral_miner_nodes_healthy", "Number of healthy nodes");
        describe_gauge!(
            "cathedral_miner_nodes_unhealthy",
            "Number of unhealthy nodes"
        );
        describe_counter!(
            "cathedral_miner_node_health_checks_total",
            "Total node health checks performed"
        );
        describe_histogram!(
            "cathedral_miner_node_health_check_duration_seconds",
            "Duration of node health checks"
        );

        // Validator interaction metrics
        describe_counter!(
            "cathedral_miner_validator_requests_total",
            "Total requests from validators"
        );
        describe_counter!(
            "cathedral_miner_validator_auth_failures_total",
            "Total validator authentication failures"
        );
        describe_gauge!(
            "cathedral_miner_validator_sessions_active",
            "Currently active validator sessions"
        );
        describe_histogram!(
            "cathedral_miner_validator_session_duration_seconds",
            "Duration of validator sessions"
        );
        describe_counter!(
            "cathedral_miner_validator_node_discoveries_total",
            "Total node discovery requests"
        );

        // SSH management metrics
        describe_counter!(
            "cathedral_miner_ssh_sessions_created_total",
            "Total SSH sessions created"
        );
        describe_counter!(
            "cathedral_miner_ssh_sessions_closed_total",
            "Total SSH sessions closed"
        );
        describe_gauge!(
            "cathedral_miner_ssh_sessions_active",
            "Currently active SSH sessions"
        );
        describe_histogram!(
            "cathedral_miner_ssh_session_duration_seconds",
            "Duration of SSH sessions"
        );
        describe_counter!(
            "cathedral_miner_ssh_key_deployments_total",
            "Total SSH key deployments"
        );
        describe_counter!(
            "cathedral_miner_ssh_failures_total",
            "Total SSH operation failures"
        );

        // Deployment metrics
        describe_counter!(
            "cathedral_miner_deployment_attempts_total",
            "Total deployment attempts"
        );
        describe_counter!(
            "cathedral_miner_deployment_failures_total",
            "Total deployment failures"
        );
        describe_histogram!(
            "cathedral_miner_deployment_duration_seconds",
            "Duration of deployment operations"
        );
        describe_gauge!(
            "cathedral_miner_remote_nodes_deployed",
            "Number of remotely deployed nodes"
        );

        // Database metrics
        describe_counter!(
            "cathedral_miner_database_operations_total",
            "Total database operations"
        );
        describe_counter!(
            "cathedral_miner_database_errors_total",
            "Total database errors"
        );
        describe_histogram!(
            "cathedral_miner_database_query_duration_seconds",
            "Database query duration"
        );
        describe_gauge!(
            "cathedral_miner_database_connections_active",
            "Active database connections"
        );

        // Bittensor integration metrics
        describe_counter!(
            "cathedral_miner_bittensor_registrations_total",
            "Total Bittensor network registrations"
        );
        describe_counter!(
            "cathedral_miner_bittensor_errors_total",
            "Total Bittensor operation errors"
        );
        describe_gauge!("cathedral_miner_bittensor_uid", "Current Bittensor UID");
        describe_gauge!("cathedral_miner_bittensor_stake", "Current stake amount");
        describe_counter!(
            "cathedral_miner_axon_requests_total",
            "Total axon server requests"
        );

        // System metrics
        describe_gauge!("cathedral_miner_uptime_seconds", "Miner uptime in seconds");
        describe_gauge!("cathedral_miner_memory_usage_bytes", "Memory usage in bytes");
        describe_gauge!("cathedral_miner_cpu_usage_percent", "CPU usage percentage");
    ...
    pub fn record_node_health_check(
        ...
        counter!("cathedral_miner_node_health_checks_total").increment(1);
        histogram!("cathedral_miner_node_health_check_duration_seconds")
    ...
    pub fn update_node_counts(&self, total: u64, healthy: u64, unhealthy: u64) {
        gauge!("cathedral_miner_nodes_total").set(total as f64);
        gauge!("cathedral_miner_nodes_healthy").set(healthy as f64);
        gauge!("cathedral_miner_nodes_unhealthy").set(unhealthy as f64);
    }
    ...
    pub fn record_validator_request(
        ...
        counter!("cathedral_miner_validator_requests_total").increment(1);

        if !success {
            counter!("cathedral_miner_validator_auth_failures_total").increment(1);
        }
    ...
    pub fn record_ssh_session_created(&self, _node_id: &str, _validator_hotkey: &str) {
        counter!("cathedral_miner_ssh_sessions_created_total").increment(1);
    }
    ...
    pub fn record_ssh_session_closed(
        ...
        counter!("cathedral_miner_ssh_sessions_closed_total").increment(1);
        histogram!("cathedral_miner_ssh_session_duration_seconds").record(duration.as_secs_f64());
    }

    /// Update active SSH sessions count
    pub fn set_active_ssh_sessions(&self, count: u64) {
        gauge!("cathedral_miner_ssh_sessions_active").set(count as f64);
    }

    /// Record SSH key deployment
    pub fn record_ssh_key_deployment(&self, _node_id: &str, success: bool, _operation_type: &str) {
        counter!("cathedral_miner_ssh_key_deployments_total").increment(1);

        if !success {
            counter!("cathedral_miner_ssh_failures_total").increment(1);
        }
    }
    ...
    pub fn record_deployment(
        ...
        counter!("cathedral_miner_deployment_attempts_total").increment(1);
        histogram!("cathedral_miner_deployment_duration_seconds").record(duration.as_secs_f64());

        if !success {
            counter!("cathedral_miner_deployment_failures_total").increment(1);
        }
    ...
    pub fn set_remote_nodes_deployed(&self, count: u64) {
        gauge!("cathedral_miner_remote_nodes_deployed").set(count as f64);
    }

    /// Record database operation
    pub fn record_database_operation(&self, _operation: &str, success: bool, duration: Duration) {
        counter!("cathedral_miner_database_operations_total").increment(1);
        histogram!("cathedral_miner_database_query_duration_seconds").record(duration.as_secs_f64());

        if !success {
            counter!("cathedral_miner_database_errors_total").increment(1);
        }
    }

    /// Set active database connections
    pub fn set_database_connections(&self, count: u64) {
        gauge!("cathedral_miner_database_connections_active").set(count as f64);
    }

    /// Record Bittensor registration
    pub fn record_bittensor_registration(&self, success: bool, uid: Option<u16>) {
        counter!("cathedral_miner_bittensor_registrations_total").increment(1);

        if let Some(uid_value) = uid {
            gauge!("cathedral_miner_bittensor_uid").set(uid_value as f64);
        }

        if !success {
            counter!("cathedral_miner_bittensor_errors_total").increment(1);
        }
    }

    /// Update Bittensor stake
    pub fn set_bittensor_stake(&self, stake: u64) {
        gauge!("cathedral_miner_bittensor_stake").set(stake as f64);
    }

    /// Record axon request
    pub fn record_axon_request(&self, _method: &str, success: bool) {
        counter!("cathedral_miner_axon_requests_total").increment(1);
    ...
    async fn try_collect_periodic_metrics(&self) -> Result<()> {
        ...
        gauge!("cathedral_miner_uptime_seconds").set(uptime);

        // Collect system metrics
        if let Ok(memory_usage) = self.get_memory_usage().await {
            gauge!("cathedral_miner_memory_usage_bytes").set(memory_usage as f64);
        }

        if let Ok(cpu_usage) = self.get_cpu_usage().await {
            gauge!("cathedral_miner_cpu_usage_percent").set(cpu_usage);
        }
