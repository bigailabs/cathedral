...
use basilica_miner::config::{NodeConfig, NodeManagementConfig, MinerConfig};
use basilica_miner::node_fleet_manager::NodeFleetManager;
...
use basilica_protocol::basilica_miner::{ListNodesRequest, SshAccessRequest, ValidatorAuthRequest};
...
#[tokio::test]
async fn test_list_nodes_no_session() {
    let config = create_test_miner_config();
    let fleet_manager = create_test_fleet_manager().await;
    let db = create_test_db().await;
    ...
    let response = service.list_nodes(request).await;
    assert!(response.is_err());
    assert_eq!(response.unwrap_err().code(), tonic::Code::Unauthenticated);
}
...
async fn create_test_fleet_manager() -> NodeFleetManager {
    let config = NodeManagementConfig {
    ...
    let db = create_test_db().await;
    NodeFleetManager::new(config, db).await.unwrap()
}
