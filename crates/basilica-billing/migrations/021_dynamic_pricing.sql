...
-- ===========================================================================
-- 3. Alter billing_packages table for dynamic pricing
-- ===========================================================================

-- Add column to track if dynamic pricing is enabled for this package
ALTER TABLE billing.billing_packages
ADD COLUMN IF NOT EXISTS use_dynamic_pricing BOOLEAN DEFAULT false;

-- Add column to store last synced market price
ALTER TABLE billing.billing_packages
ADD COLUMN IF NOT EXISTS last_market_price DECIMAL(10, 4);

-- Add column to track last price update timestamp
ALTER TABLE billing.billing_packages
ADD COLUMN IF NOT EXISTS price_last_updated_at TIMESTAMP WITH TIME ZONE;

-- Column comments
COMMENT ON COLUMN billing.billing_packages.use_dynamic_pricing IS
    'If true, hourly_rate is updated from price_cache; if false, use static hourly_rate';

COMMENT ON COLUMN billing.billing_packages.last_market_price IS
    'Last fetched market price (before discount) for tracking';

COMMENT ON COLUMN billing.billing_packages.price_last_updated_at IS
    'Timestamp of last price sync from external sources';


-- ===========================================================================
-- 4. Enable dynamic pricing for main GPU packages
-- ===========================================================================

-- Enable dynamic pricing for major GPU packages
UPDATE billing.billing_packages
SET use_dynamic_pricing = true
WHERE package_id IN ('h100', 'h200', 'a100', 'b200')
    AND active = true;
